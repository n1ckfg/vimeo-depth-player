...
var DepthPlayer = function (_EventEmitter) {
    ...
    _createClass(DepthPlayer, [{
        ...
        key: 'buildGeometry',
        value: function buildGeometry(eqr) {
		    DepthPlayer.geo = new THREE.Geometry();
        	eqr=true;
            if (eqr) {
            	var detail = VERTS_WIDE;
    			for (var m = 0; m < detail; m++) {
			      	for (var n = 0; n < detail; n++) {
				        var x = Math.sin(Math.PI * m / detail) * Math.cos(2.0 * Math.PI * n / detail);
				        var z = Math.sin(Math.PI * m / detail) * Math.sin(2.0 * Math.PI * n / detail);
				        var y = Math.cos(Math.PI * m / detail); // up
				          
		                DepthPlayer.geo.vertices.push(new THREE.Vector3(x, y, z));
			      	}
			    }
			    for (var _y = 0; _y < detail - 1; _y++) {
		            for (var _x8 = 0; _x8 < detail - 1; _x8++) {
